...
const { needsAzureRefresh, getNewAzureURL } = require('~/server/services/Files/Azure/crud');
...
const getUserController = async (req, res) => {
  ...
  // Handle Azure blob avatar URL refresh
  if (appConfig.fileStrategy === FileSources.azure_blob && userData.avatar) {
    const avatarNeedsRefresh = needsAzureRefresh(userData.avatar, 3600); // 1 hour buffer (matches S3)
    if (!avatarNeedsRefresh) {
      return res.status(200).send(userData);
    }
    const originalAvatar = userData.avatar;
